...
(0, _assign3.default)(Client.prototype, {
  ...
  initializePool: function initializePool(config) {
    ...
    this.pool = new _tarn.Pool((0, _assign3.default)(this.poolDefaults(config.pool || {}), { acquireTimeoutMillis: config.acquireConnectionTimeout || 60000 }, config.pool));
  },
  ...
  acquireConnection: function acquireConnection() {
    if (!this.pool) {
      return {
        completed: _bluebird2.default.reject(new Error('Unable to acquire a connection'))
      };
    } else {
      return {
        completed: this.pool.acquire().promise.then(function (connection) {
  ...
  destroy: function destroy(callback) {
    var promise = this.pool ? this.pool.destroy() : _bluebird2.default.resolve();
    this.pool = undefined;
