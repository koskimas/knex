...
var Transaction = function (_EventEmitter) {
  ...
  Transaction.prototype.acquireConnection = function acquireConnection(client, config, txid) {
    ...
    }).catch(_tarn.TimeoutError, function (error) {
      var timeoutError = new Error('Knex: Timeout acquiring a connection for a transaction. The pool is probably full. ' + 'Are you missing a .transacting(trx) call?');
